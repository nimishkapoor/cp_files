...
int fun()
{
	...
	{
		c[in]=1000;d[in]=1000;s[in]=1000;
		for(int i=0;i<m;i++)
		{
			if(v[in][i]>='a' && v[in][i]<='z')
			{
				c[in]=min(c[in],min(i,m-i));
			}
			if(v[in][i]>='0' && v[in][i]<='9')
			{
				d[in]=min(d[in],min(i,m-i));
			}
			if(v[in][i]=='#' || v[in][i]=='*' || v[in][i]=='&')
			{
				s[in]=min(s[in],min(i,m-i));
			}
		}
	}	
