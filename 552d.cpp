...
ll fun(ll x, ll b, ll a)
{
	if(x==n)
	{
		dp.insert(mp(mp(x,mp(b,a)),0));
		return 0;
	}
	auto it=dp.find(mp(x,mp(b,a)));
	if(it !=dp.end())
	{
		//watch(it->sc);
		return it->sc;
	...
	dp.insert(mp(mp(x,mp(b,a)),max(t1,t2)));
	return max(t1,t2);
}
