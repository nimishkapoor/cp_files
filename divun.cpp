...
int main()
{
	...
	while(t--)
	{
		priority_queue<pair<int,pair<int,int > > , vector<pair<int,pair<int,int> > > > h;
		int n;
		cin>>n;
		int l[n],r[n];
		for(int i=0;i<n;i++)
		{
			cin>>l[i]>>r[i];
			h.push(mp(r[i]-l[i],mp(l[i],r[i])));
		}
		int l1,r1,l2,r2,tmpl,tmpr;
		for(int i=0;i<n;i++)
		{
			auto it=h.top();
			tmpl=it.sc.fr;
			...
			else if(tmpl<=r1 && tmpr>=r1)
			{
				r1=tmpr;
			}
			else if(tmpl<=l1 && tmpr>=r1)
			{
				l1=tmpl;
				r1=tmpr;
			}
			else if(tmp1>=l1 && tmpr<=r1)
			{
