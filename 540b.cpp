...
int main()
{
	...
	if(n%2==0)
	{
		...
		for(int i=1;i<=n;i++)
		{
			...
			{
				p=even[i-2]+odd[m]-odd[i-1];
				q=odd[i-1]+even[n]-even[i];
			}
			else
			{
				p=odd[i-2]+even[n]-even[i-1];
				q=even[i-1]+odd[m]-odd[i];
			}
			//watch(p);
			//watch(q);
			if(p==q)
	...
	else
	{
		...
		for(int i=1;i<=n;i++)
		{
			...
			{
				p=even[i-2]+odd[n]-odd[i-1];
				q=odd[i-1]+even[m]-even[i];
			}
			else
			{
				p=odd[i-2]+even[m]-even[i-1];
				q=even[i-1]+odd[n]-odd[i];
			}
			//watch(p);
			//watch(q);
			if(p==q)
