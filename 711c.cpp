...
ll dp[101][101][101];
int n,k,m;
ll rec(int in, int prev, int beaut)
{
	if(beaut>k)
	{
		return inf;
	...
	if(in==n)
	{
		return (beaut==k)?0:inf;
	}
	if(dp[in][prev][beaut]!=0)
	{
		return dp[in][prev][beaut];
	}
	ll ans=inf;
	if(a[in]!=0)
	{
		ans=rec(in+1,a[in],beaut+(prev!=a[in]));
	}
	else
	{
		for(int i=0;i<m;i++)
		{
			ans=min(ans,cost[in][i]+rec(in+1,i+1,beaut+(prev!=i+1)));
		}
	}
	return dp[in][prev][beaut]=ans;
}
int main()
{
	...
	cout<<(rec(0,0,0)!=inf?dp[0][0][0]:-1)<<endl;
	return 0;
}
