...
ll c[100005];
int f=0;
ll dfs(int x,int p,int prev,int dprev)
{
	if(g[x].size()>2)
	{
		f=1;
	...
	for(int i=0;i<g[x].size();i++)
	{
		...
		if(prev==1)
		{
			return max(b[x]+dfs(y,x,2),c[x]+dfs(y,x,3));
		}
		else if(prev==2)
		{
			return max(a[x]+dfs(y,x,1),c[x]+dfs(y,x,3));
		}
		else if(prev==3)
		{
			return max(b[x]+dfs(y,x,2),a[x]+dfs(y,x,1));
		}
		else
		{
			return max3(a[x]+dfs(y,x,1),b[x]+dfs(y,x,2),c[x]+dfs(y,x,3));
		}
	}
	return 0;
}
