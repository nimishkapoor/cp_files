    ...
    while(t--)
    {
		...
		while(!min_heap.empty())
		{
			...
			if(vis[it.second]==0)
			{
				...
				{
					if(vis[r[tp][i].first]==1)
					{
						continue;
					}
					if(disr[r[tp][i].first]>disr[tp]+r[tp][i].second)
					{
						disr[r[tp][i].first]=min(disr[r[tp][i].first],disr[tp]+r[tp][i].second);
						min_heap.push(mp(disr[r[tp][i].first],r[tp][i].first));
					}
				}
			}
			min_heap.pop();
		...
		{
			cin>>x>>y>>w;
			ans=min(ans,min(dis[x]+w+disr[y],dis[y]+w+disr[x]));
		}
		if(ans==inf)
		{
			cout<<-1<<endl;
