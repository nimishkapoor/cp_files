...
int main()
{
	...
	while(n!=0)
	{
		...
		a.pb(mp(inf,a[n-1].sc+inf));
		b.pb(mp(inf,b[m-1].sc+inf));
		int last_suma=0,last_sumb=0,ans=0;
		int i=0,j=0;
		while(i<=n && j<=m)
		{
			if(a[i].fr<b[j].fr)
			...
			else if(a[i].fr>b[j].fr)
			{
				j++;
			}
			else
			{
				ans+=max(a[i].sc-last_suma,b[j].sc-last_sumb);
				last_suma=a[i].sc;
				last_sumb=b[j].sc;
				i++,j++;
			}
