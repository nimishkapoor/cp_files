	...
	while(n!=0 && m!=0)
	{
		...
		while(!min_heap.empty())
		{
			...
			{	
				...
				if(dist[g[tp][i].fr]==dist[tp]+g[tp][i].sc && g[tp][i].fr==t)
				{
					int j=tp;
					edge.pb(mp(tp,t));//watch(tp);watch(t);
					while(par[j]!=-1)
					{
						edge.pb(mp(par[j],j));//watch(par[j]);watch(j);
						j=par[j];
					}
					continue;
				}
				...
				if(dist[g[tp][i].fr]>dist[tp]+g[tp][i].sc)
				{
					...
					min_heap.push(mp(dist[g[tp][i].fr],g[tp][i].fr));
					par[g[tp][i].fr]=tp;
		...
		while(!min_heap.empty())
		{
			auto it=min_heap.top();
				...
				for(int i=0;i<g[tp].size();i++)
				{	
					if(find(all(edge),mp(tp,g[tp][i].fr))==edge.end())
					{//watch(tp);watch(g[tp][i].fr);
						if(dist[g[tp][i].fr]>dist[tp]+g[tp][i].sc)
						{
							dist[g[tp][i].fr]=dist[tp]+g[tp][i].sc;
							min_heap.push(mp(dist[g[tp][i].fr],g[tp][i].fr));
