...
ll fun(ll i,ll prev)
{
	...
	if(s[i]=='0')
	{
		if(prev==0)
		{
			return dp[i][prev]=a+b+fun(i+1,0);
		}
		else
		{
			if(i+1==n)
			{
				return dp[i][prev]=2*a+2*b+fun(i+1,0);
			}
			else if(s[i+1]=='0')
			{
				return dp[i][prev]=min(a+2*b+fun(i+1,1),2*a+2*b+fun(i+1,0));
			}
			else
			{
				return dp[i][prev]=a+2*b+fun(i+1,1);
			}
		}
	}
	...
	if(s[i]=='1')
	{
		...
		{
			return dp[i][prev]=a*2+b*2+fun(i+1,1);
		}
	}
}
int main()
