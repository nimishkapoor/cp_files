...
const int inf = INT_MAX;//0x3f3f3f3f;//(ll) 9e18
	...
	while(!q.empty())
	{
		auto it=q.front();
			...
			for(int i=0;i<4;i++)
			{
				int a=it.fr+dx[i];
				int b=it.sc+dy[i];
				if(a>=0 && a<n && b>=0 && b<m  && g[a][b]!='x' && lvl[a][b]==inf)
				{
					q.push(mp(a,b));
					lvl[a][b]=lvl[it.fr][it.sc]+1;
				}
			}
...
int tsp(int s,int x,int mask)
{
	...
	for(int i=0;i<x;i++)
	{
		if(mask & (1<<i))
		{
			ret=min(ret,tsp(i,x,mask ^(1<<i))+tdis[s][i]);
		}
	}
	return ret;
}
int main()
{
	...
	while(n!=0 && m!=0)
	{
		...
		{
			bfs(dirty[i].fr,dirty[i].sc);
			for(int j=0;j<dirty.size();j++)
			{
				tdis[i][j]=lvl[dirty[j].fr][dirty[j].sc];
				if(tdis[i][j]==inf)
				{
					f=1;
				}
		...
		{
			int x;
			int mask=pow(2,dirty.size())-1;
			mask = mask ^ 1<<(dirty.size()-1);
			cout<<tsp(dirty.size()-1,dirty.size(),mask)<<endl;
		}
		else
		{
			cout<<-1<<endl;
