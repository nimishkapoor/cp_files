...
int main()
{
	...
	while(q--)
	{
		...
		int x=1;
		int ans=0;if(x==1){watch(o);watch(z);}
		
		for(int i=0;i<n;i++)
		{if(x==1){watch(a[i]);watch(o);watch(z);}
			if(a[i]<=o)
			{
				o-=a[i];if(x==1){watch(1);}
				ans++;
			}
			else if(a[i]<=z)
			{
				z-=a[i];if(x==1){watch(2);}
				ans++;
			}
			else
			{
